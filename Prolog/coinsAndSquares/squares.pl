...
nonoverlapping(_,[S|Sides],[R|RowVars],[C|ColVars]):-
    ...
    nonoverlappingWithSide(S,R,C,Sides,RowVars,ColVars).

nonoverlappingWithSide(_,_,_,[],[],[]).
nonoverlappingWithSide(S1,R1,C1,[S2|Sides],[R2|RowVars],[C2|ColVars]):-
    R2 #>= R1 + S1 #\/ C2 #>= C1 + S1 #\/ R1 #>= R2 + S2 #\/ C1 #>= C2 + S2,
    nonoverlappingWithSide(S1,R1,C1,Sides,RowVars,ColVars).
...
main:- 
    ...
    concat(RowVars,ColVars,Vars),labeling([ff],Vars),
